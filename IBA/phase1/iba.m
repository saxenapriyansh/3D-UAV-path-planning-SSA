function path = iba(map, start, goal)
...
while i<=nPop
    ...
    %fprintf('%d %d\n',curNodeIndex, nodes);
       ...
       bat(i).velocity = 0;
       bat(i).NV = NVNodes;
       turns  = calc_turns(bat(i).Position(:,1:3));
       bat(i).Cost = k1*size(paths,1) + k2*pdist2(AllPoints(CurrentNode,:),AllPoints(GoalNode,:)) + k3*turns;
...
%         end
        ...
        [newPath,Cost] = changePath(bat(i).Position(:,4),GlobalBest.Position(:,4),AllPoints(:,:),CollisionTest,StartNode,GoalNode,nodes,Boundaryinitial,Boundaryfinal,size(DummyX,1),size(DummyY,1),size(DummyZ,1));
        ...
        if( rand > r )
            [newPath,Cost] = changePath(GlobalBest.Position(:,4),[],AllPoints(:,:),CollisionTest,StartNode,GoalNode,nodes,Boundaryinitial,Boundaryfinal,size(DummyX,1),size(DummyY,1),size(DummyZ,1));                 
        else
            [newPath,~] = changePath(bat(r2).Position(:,4),bat(r3).Position(:,4),AllPoints(:,:),CollisionTest,StartNode,GoalNode,nodes,Boundaryinitial,Boundaryfinal,size(DummyX,1),size(DummyY,1),size(DummyZ,1));    
            [newPath,Cost] = changePath(bat(r1).Position(:,4),newPath,AllPoints(:,:),CollisionTest,StartNode,GoalNode,nodes,Boundaryinitial,Boundaryfinal,size(DummyX,1),size(DummyY,1),size(DummyZ,1));    
        end
        newPath = removeLoops(newPath);
