function [path,visited_nodes] = gso(map, start, goal,visited_nodes)
    ...
    for i=1:nPop
        ...
        if size(neighbors,1) > 0
         ...
         % follow
            ...
            [newPath,Cost] = changePath(glowworms(:,4),toward(:,4),AllPoints(:,:),CollisionTest,StartNode,GoalNode,nodes,Boundaryinitial,Boundaryfinal,size(DummyX,1),size(DummyY,1),size(DummyZ,1),visited_nodes);
        ...
        elseif size(neighbors)== 0
          [newPath,Cost] = changePath(glowworm(i).Position(:,4),[],AllPoints(:,:),CollisionTest,StartNode,GoalNode,nodes,Boundaryinitial,Boundaryfinal,size(DummyX,1),size(DummyY,1),size(DummyZ,1),visited_nodes);
          newPath = removeLoops(newPath);  
           glowworm(i).Position = AllPoints(newPath(:,1),:);
            glowworm(i).Position(:,4) = newPath(:,1);
            glowworm(i).Cost = Cost;
